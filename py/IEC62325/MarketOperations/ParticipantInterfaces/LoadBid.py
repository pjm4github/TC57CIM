...
class LoadBid(ResourceBid):
    def __init__(self) -> None:
        ...
        self.load_red_initiation_time: Optional[float] = 0.0  # Load reduction initiation time
        ...
        self.metered_value: Optional[bool] = False  # Flag indicated that the load reduction is metered (See above). If priceSetting and meteredValue both equal 1, then the facility is eligible to set LMP in the real time market.
        ...
        self.min_load_reduction_interval: Optional[float] = 0.0  # Shortest period load reduction shall be maintained before load can be restored to normal levels
        self.min_time_bet_load_red: Optional[float] = 0.0  # Shortest time that load shall be left at normal levels before a new load reduction
        self.pick_up_ramp_rate: Optional[ActivePowerChangeRate] = ActivePowerChangeRate()  # Maximum rate load may be restored (MW/minute)
        self.price_setting: Optional[bool] = False  # Flag to indicate that the facility can set LMP. Works in tandem with Metered Value
        self.req_notice_time: Optional[float] = 0.0  # Time period that is required from an order to reduce a load to the time that it takes to get to the minimum load reduction
