...
class ContractRight:
    ...
    """
    def __init__(self):
        self.chain_order = 0  # When used in conjunction with a Transmission Right contract chain, this is the precedence for the contracts.
        ...
        self.financial_rights_rtm = YesNo.NO  # Flag to indicate this contract provides financial rights in the RT Market
        self.fuel_adder = 0.0 # Estimated Fuel Adder
        self.latest_sched_minutes = 0  # This indicates the latest schedule minutes (e.g. t - xx) that this resource can be notified to respond. This attribute is only used if the market type is not supplied.
        self.latest_sched_mkt_type = MarketType.DAM  # This indicates the latest schedule market type a contract can be applied to. This is used in conjunction with the latestSchedMinutes attribute to determine the latest time this contract can be called in. The possible values for this attribute are: DAM, RTM or it can be omitted. If omitted, the latestSchedMinutes attribute defines the value.
        ...
        self.tr_type = TRType.CHAIN  # Transmission Right type - is this an individual contract right or a chain contract right. Types = CHAIN or INDIVIDUAL
