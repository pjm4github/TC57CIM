...
class MssZone(AggregateNode):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.ignore_lossesOptional[YesNo] = YesNo.NO  # Provides an indication if losses are to be ignored for this metered subsystem zone.
        self.loss_factorOptional[float] = 0.0  # This is the default loss factor for the Metered Sub-System (MSS) zone. The actual losses are calculated during the RT market.
        self.ruc_gross_settlementOptional[YesNo] = YesNo.NO  # Metered Sub-System (MSS) Load Following may select Net vs. Gross settlement. Net Settlement requires the net Demand settled at the Metered Sub-Sustem (MSS) Load Aggregation Point (LAP) and Net Supply needs to settle at the equivalent to the weighted average price of the MSS generation. Gross load will be settled at the System LAP and the Gross supply will be settled at the LMP. MSS Aggregation that elects gross settlement shall have to identify if its resources are Load Following or not.
