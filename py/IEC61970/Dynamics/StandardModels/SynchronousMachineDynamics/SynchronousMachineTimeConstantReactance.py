...
class SynchronousMachineTimeConstantReactance(SynchronousMachineDetailed):
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__()
        self.ks: float = 0.0  # Saturation loading correction factor (Ks) (>= 0). Used only by Type J model. Typical
        # Value = 0.
        self.model_type: Optional[
            SynchronousMachineModelKind] = SynchronousMachineModelKind.SUBTRANSIENT_TYPE_F  # Type of synchronous
        # machine model used in Dynamic simulation applications.
        ...
        self.tpdo: Seconds = Seconds(
            5)  # Direct-axis transient rotor time constant (T'do) (> T''do). Typical Value = 5.
        self.tppdo: Seconds = Seconds(
            0.03)  # Direct-axis subtransient rotor time constant (T''do) (> 0). Typical Value = 0.03.
        self.tppqo: Seconds = Seconds(
            0.03)  # Quadrature-axis subtransient rotor time constant (T''qo) (> 0). Typical Value = 0.03.
        self.tpqo: Seconds = Seconds(
            0.5)  # Quadrature-axis transient rotor time constant (T'qo) (> T''qo). Typical Value = 0.5.
        self.x_direct_subtrans: float = 0.2  # Direct-axis subtransient reactance (unsaturated) (X''d) (> Xl).
        # Typical Value = 0.2.
        self.x_direct_sync: float = 1.8  # Direct-axis synchronous reactance (Xd) (>= X'd). Typical Value = 1.8.
        self.x_direct_trans: float = 0.5  # Direct-axis transient reactance (unsaturated) (X'd) (>= X''d). Typical
        # Value = 0.5.
