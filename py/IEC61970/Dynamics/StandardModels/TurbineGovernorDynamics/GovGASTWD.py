...
class GovGastwd(TurbineGovernorDynamics):
    ...
    def __init__(self) -> None:
        ...
        self.bf2: PU = PU(
            1.0)  # Turbine Torque Coefficient K_hhv (depends on heating value of fuel stream in combustion chamber)
        # (Bf2).
        self.c: float = 1.0  # Valve positioner (C)
        self.cf2: PU = PU(
            1.0)  # Coefficient defining fuel flow where power output is 0% (Cf2). Synchronous but no output.
        # Typically 0.23 x K_hhv (23% fuel flow).
