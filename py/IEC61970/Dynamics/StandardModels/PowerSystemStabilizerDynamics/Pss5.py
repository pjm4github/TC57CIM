...
class Pss5(PowerSystemStabilizerDynamics):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.ctw2: bool = True  # Selector for Second washout enabling (C_TW2). True = second washout filter is bypassed, False = second washout filter in use. Typical Value = True.
        self.deadband: PU = PU(0.0)  # Stabilizer output dead band (DeadBand). Typical Value = 0.
        self.isfreq: bool = True  # Selector for Frequency/shaft speed input (IsFreq). True = speed, False = frequency. Typical Value = True.
        self.kf: float = 5.0  # Frequency/shaft speed input gain (K_F). Typical Value = 5.
        self.kpe: float = 0.3  # Electric power input gain (K_PE). Typical Value = 0.3.
        self.kpss: float = 1.0  # PSS gain (K_PSS). Typical Value = 1.
        ...
        self.vadat: bool = False  # Signal selector (V_adAtt). True = closed (Generator Power is greater than P_min), False = open (Pe is smaller than P_min). Typical Value = True.
