...
class Pss1(PowerSystemStabilizerDynamics):
    ...
    def __init__(self):
        super().__init__()
        self.kf: Optional[float] = 5.0  # Frequency power input gain (K<sub>EF</sub>). Typical Value = 5.
        self.kpe: Optional[float] = 0.3  # Electric power input gain (K<sub>EPE</sub>). Typical Value = 0.3.
        self.ks: Optional[float] = 1.0  # PSS gain (K<sub>ES</sub>). Typical Value = 1.
        self.kw: Optional[float] = 0.0  # Shaft speed power input gain (K<sub>EW</sub>). Typical Value = 0.
