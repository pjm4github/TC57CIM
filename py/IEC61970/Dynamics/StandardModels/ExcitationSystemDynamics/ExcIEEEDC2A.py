...
class ExcIeeeDc2A(ExcitationSystemDynamics):
    ...
    def __init__(self) -> None:
        ...
        self.exclim: float = 0.0  # (exclim). IEEE standard is ambiguous about lowering limit on exciter output.
        ...
        # T_E). Typical Value = 1.33
        self.tf: Seconds = Seconds()  # Excitation control system stabilizer time constant (T_F). Typical Value = 0.675
        self.uelin: bool = False  # UEL input (uelin). true = input is connected to the HV gate, false = input
        # connects to the error signal. Typical Value = True
